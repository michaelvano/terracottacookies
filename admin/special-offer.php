...
// META DESCRIPTIONS
$title = "Manage Special Offer";
$mdesc = "";	
$mbots = "";
$canon = "";
$page = "admin";
$subPage = "special";
$thisPage = "/admin/special-offer.php";
...
if ($_action == "insert" || $_action == "update")
{
	...
	if ($_action == "update") { $old_image = getSingleValue("image", $database_table, "id", 1); }
	...
	$fields = array("content", "image");
	$values = array($postContent, $new_image_name);
	
	if (!$content_needed && !$too_big && !$invalid_formats)
	{	
		// update information
		if ($_action == "update")
		{
			$update = updateInfo($database_table, $fields, $values, "id", 1);
...
else
{
	...
	<div class="mainContent">
		...
		if ($_action == "add" || $_action == "edit") 
		{
			if ($_action == "edit")
			{
				$get = getInfo($database_table, "id", 1);
				extract($get[0]);
			}
