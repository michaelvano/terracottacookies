<%
    ...
    if infoFilled=0 then 
		...
		if not Len(Trim(Request.Form("7002")))=0 then 
		myBody=myBody & "7002 Oatmeal Chocolate Chip Small, Bulk(96/case) = " & Request.Form("7002") & " case(s)" & chr(13) &Chr(10)
)	
        end if
		if not Len(Trim(Request.Form("7102")))=0 then 
		myBody=myBody & "7102 Fudge Chip Mookie Small, Bulk(96/case) = " & Request.Form("7102") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("7003")))=0 then 
		myBody=myBody & "7003 Oatmeal Chocolate Chip Large, Wrapped(48/case) = " & Request.Form("7003") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("7103")))=0 then 
		myBody=myBody & "7103 Fudge Chip Mookie Large, Wrapped(48/case) = " & Request.Form("7103") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("7004")))=0 then 
		myBody=myBody & "7004 Oatmeal Chocolate Chip Large, Bulk(48/case) = " & Request.Form("7004") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("7104")))=0 then 
		myBody=myBody & "7104 Fudge Chip Mookie Large, Bulk(48/case) = " & Request.Form("7104") & " case(s)" & chr(13) &Chr(10)
)		
		end if
		...
		if not Len(Trim(Request.Form("GF-10")))=0 then 
		myBody=myBody & "GF-10 Gluten Free Chocolate Chip Cookies ½ lb bag, 225 gram = " & Request.Form("GF-10") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("GF-15")))=0 then 
		myBody=myBody & "GF-15 Gluten Free Brownies 7 ½ “ pan, 325 gram = " & Request.Form("GF-15") & " case(s)" & chr(13) &Chr(10)
)
end if
		if not Len(Trim(Request.Form("GF-20")))=0 then 
		myBody=myBody & "GF-20 Gluten Free Vanilla Cake 8 ” pan, 450 gram = " & Request.Form("GF-20") & " case(s)" & chr(13) &Chr(10)
)
		end if
...
'		MyCDONTSMail.bcc= "cmaxwell@maxmediagroup.com"
		MyCDONTSMail.To= "orders@terracottacookies.com"
'		MyCDONTSMail.To= "catalinst@rogers.com"
		MyCDONTSMail.From= "orders@terracottacookies.com"
'		MyCDONTSMail.cc = Request.Form("cemail")
		MyCDONTSMail.Subject="TerraCotta Cookies Confirmation"
		MyCDONTSMail.Body= MyBody
		MyCDONTSMail.Send
		set MyCDONTSMail=Nothing
		Set MyCDONTSMail = Server.CreateObject("CDONTS.NewMail")
'		MyCDONTSMail.bcc= "cmaxwell@maxmediagroup.com"
		MyCDONTSMail.To= Request.Form("cemail")
'		MyCDONTSMail.To= "catalinst@rogers.com"
		MyCDONTSMail.From= "orders@terracottacookies.com"
'		MyCDONTSMail.cc = Request.Form("cemail")
		MyCDONTSMail.Subject="TerraCotta Cookies Confirmation"
